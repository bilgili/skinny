...
| Constraint | Detail |
|------------|--------|
| State | **Wired + GPU-validated.** Megakernel spectral integrators `integrators/path_spectral.slang` (`SpectralPathTracer`) and `integrators/bdpt_spectral.slang` (`SpectralBDPTIntegrator`, change `spectral-bdpt-megakernel`), compiled under `-DSKINNY_SPECTRAL`, render the path/bdpt+megakernel+flat envelope on both backends: per-λ NEE, pbrt sigmoid/D65 upsampling (`spectrum.slang`), exact named-conductor Fresnel (6.2, binding 48), authored illuminant SPDs (6.3, binding 50) + blackbody Planck emission (6.1, bindings 49/51), hero-λ glass dispersion (6.4). Shared per-λ flat NEE lives in `integrators/spectral_flat_common.slang`. Foundation (upsampling/CIE tables `pbrt/data/`, numpy mirror `pbrt/spectral.py`, import payload preservation, `spectrum.slang`) unchanged. |
...
| Scope | **Path + BDPT + megakernel + flat materials only.** No wavefront (follow-up), no SPPM (no megakernel path — spectral SPPM awaits spectral wavefront), no neural proposal, no ReSTIR reuse, no skin/subsurface/volume — each refused at startup. `--integrator bdpt` runs `SpectralBDPTIntegrator` (all five strategy families spectral; scalar MIS reused verbatim from `bdpt.slang`); path/bdpt switch live in a spectral session, sppm pins to path. 4 hero-rotated wavelengths (pbrt visible-λ pdf); CIE film resolve to the existing RGBA32F accumulation. |
