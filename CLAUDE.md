...
| MLT integrator — PSSMLT over BDPT (wavefront, flat, RGB) | ✅ (`WavefrontMltPass`) | ✅ (`MetalWavefrontMltPass`, `mlt-integrator`; bit-identical at equal budget) |
...
| Spectral rendering (`--spectral`, hero-wavelength) | ✅ (path + bdpt + sppm, megakernel + wavefront, flat, v1) | ✅ (`spectral-rendering`, `spectral-bdpt-megakernel`, `spectral-wavefront`) |
...
| Backends | Vulkan (`WavefrontMltPass`) and native Metal (`MetalWavefrontMltPass`) — bit-identical at equal budget (measured relMSE 0.0983, mean 0.251768 on the `int_caustic` suite scene). Metal watchdog: chain-mutation dispatches breadth-tiled + flushed per sub-batch under `SKINNY_METAL` (one mutation = one complete BDPT sample per chain). |
