"""Command queue and renderer proxy shared by every front-end.

Front-end-neutral by construction: this module imports no GUI toolkit, so it can
be unit-tested without a GUI or GPU, and any caller that is not the renderer's
owning thread can marshal through it. Callers post renderer mutations; the
thread that owns frame rendering runs them via :meth:`RenderCommandQueue.run_pending`.

Historically this lived under ``skinny.ui.qt``; that path still re-exports these
names.
"""
...
class RenderCommandQueue:
    ...
    def drain(self) -> list[RenderCommand]:
        """Remove and return pending commands without executing them.

        Prefer :meth:`run_pending`. A caller that drains and loops the callbacks
        itself must remember to settle every ``reply`` future, or awaited
        commands hang until their timeout.
        """
        with self._lock:
    ...
    def run_pending(
        ...
        *,
        ...
        """Execute pending commands against ``target`` and settle their replies.

        Called by whichever thread owns ``target``. Every command's reply future
        is completed with its result or its exception, so a caller blocked on
        ``post_with_reply`` is never left waiting on a command that already ran.
        """
        for command in self.drain():
            # A caller that timed out may have cancelled its command; running it
            # anyway would apply a mutation the caller was told had no effect.
            if command.reply is not None and not command.reply.set_running_or_notify_cancel():
                continue
            try:
                result = command.callback(target)
