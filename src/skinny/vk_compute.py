...
class ComputePipeline:
    ...
    def __init__(
        ...
        *,
        ...
        # scene set must carry them for the MLT pipeline layout to be
        # compatible. The megakernel layout (and its SPIR-V) is unchanged.
        self.mlt_bindings = not compile_pipeline
    ...
    def _create_descriptor_set_layout(self):
        ...
        # writes dummies until `_ensure_wavefront_mlt_pass` rebinds the real
        # chain buffers.
        #
        # The numbers come from `wavefront_layout.MLT_CHAIN_BUFFERS` (change
        # mlt-binding-declaration), never restated here: a layout that omits a
        # binding the shader references is undefined behaviour on Vulkan and a
        # hard `SPIR-V to MSL conversion error: nullptr` on MoltenVK — the exact
        # failure a hand-maintained copy produced for `volumeDensity` (change
        # fix-vulkan-volume-density-binding).
        if self.mlt_bindings:
            for _mlt_binding in mlt_binding_numbers():
                bindings.append(
                    vk.VkDescriptorSetLayoutBinding(
