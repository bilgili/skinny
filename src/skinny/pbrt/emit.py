...
def add_mesh(stage, path, points, indices, *, normals=None, uvs=None) -> UsdGeom.Mesh:
...
def tessellate_sphere(radius: float, *, segments: int = 32, rings: int = 16):
    """UV-sphere -> (points (N,3), tri-indices (M,3), normals (N,3))."""
    pts = []
    nrm = []
    ...
    return np.array(pts), np.array(idx, dtype=np.int64), np.array(nrm)
