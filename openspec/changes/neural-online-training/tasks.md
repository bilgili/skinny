...
- [~] 7.3 Benchmark harness `tests/bench_neural_online.py` (task-7.3). **Measured on the 4090:** the file-handoff online swap runs end to end — 32 swaps, version++ each frame, publish ≈ 25 ms (the NFW1 CPU round-trip the interop path removes), render+swap ≈ 31 ms median. CUDA trainer cycle validated separately (2.2). The **interop CUDA-write seam (5.2) is now implemented and verified** end to end on the 4090 (`tests/test_neural_interop.py`: the renderer online loop publishes weights into the CUDA-shared buffers and the timeline-gated frame-end swap bumps `networkVersion` with no CPU re-upload). **Still pending** (needs torch in the *renderer* venv): the quantified file-vs-interop publish-latency comparison, CUDA training concurrent with the Vulkan render in one process, and frames-to-recover on a moving object — documented in the harness
