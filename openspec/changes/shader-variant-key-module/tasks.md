...
- [ ] 1.1 Record, for the current tree, a golden snapshot per compile site:
...
- [ ] 1.2 Record blake2b `_cache_key` values for the Vulkan megakernel and
      preview pipelines over a pinned source tree (cache-hit check for 4.2).

## 2. Module + tests (no consumers yet)

- [ ] 2.1 Add `src/skinny/shader_variants.py`: `Target`/`Family` enums
...
- [ ] 2.2 Add `tests/test_shader_variants.py`: cross-backend agreement sweep
...
- [ ] 2.3 Verify the design D5 asymmetry record against the 1.1 goldens
...
- [ ] 3.1 Migrate `vk_compute.ComputePipeline._compile_slang` and
...
- [ ] 3.2 Verify cache-hit incl. the spectral megakernel: `_cache_key` values
      equal 1.2 and a rebuild over an unchanged tree reuses the cached `.spv`
      without invoking `slangc`.
- [ ] 3.3 Migrate `vk_wavefront._slang_flags` (`SKINNY_WAVEFRONT` spliced
...
- [ ] 4.1 Migrate the three `metal_compute` define-literal sites to
      `session_defines()` (full-dict single assignment — SlangPy
      `opts.defines` copy-on-read gotcha); assert dicts equal the 1.1 goldens.
- [ ] 4.2 Migrate `metal_wavefront._metal_slang_session` and the five
...
- [ ] 6.2 GPU smoke under dispatch-hygiene rules (one guarded Metal process):
      ...
      `build/spv_cache`.
- [ ] 6.3 Byte-identity confirmation: recompile the megakernel and the 28 RGB
      wavefront kernels pre/post on the same slangc — `.spv` bytes identical
      (guarantees 1–4 of the spec requirement).
- [ ] 6.4 Update `docs/Architecture.md` with a short "Shader variant key"
