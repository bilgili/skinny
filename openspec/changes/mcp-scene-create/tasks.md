...
- [ ] 2.1 Add `allow_empty: bool = False` to `load_scene_from_stage` /
...
- [ ] 2.2 Pass `allow_empty=True` from `_resync_geometry_from_stage` so
  remove-last-instance no longer raises there (latent bug) and empty-scene
  resync works.
- [ ] 2.3 In `_upload_usd_scene`'s `if not scene.instances:` branch, reset the
...
- [ ] 3.1 Add `create_empty_scene()` to `renderer.py`: build an in-memory stage
  (`Usd.Stage.CreateInMemory()`), author `/World` Xform as default prim, set
  up-axis Y and metersPerUnit 1.
- [ ] 3.2 Adopt the stage: `_usd_scene = load_scene_from_stage(stage,
...
- [ ] 3.3 Reset stale per-scene state for force-replace: `_usd_up_axis_rt`,
  `_anim_index`, `_skeletal`, `_usd_controls`, `clock`, `_usd_bake_done` to
  their defaults.
- [ ] 3.4 Finalize via `_resync_geometry_from_stage()` (scene-graph build +
  synthetic light/dome/camera injection + upload + version bump).

## 4. MCP tool: scene_create

- [ ] 4.1 Add `scene_create(self, force: bool = False)` to `mcp_server.py` with a
...
- [ ] 4.2 Register the tool in the build_app tool tuple (no `__signature__`
  special-casing — `_wrap` sets it generically for every tool).

## 5. Tests

- [ ] 5.1 Hostless unit for `create_empty_scene()`: after the call, `_usd_stage`
  and `_usd_edit_layer` are set, the scene graph contains `/World`, and an empty
  `Scene` (no instances) results without raising.
- [ ] 5.2 Extend the MCP tool-schema test to assert a `scene_create` tool with a
  `force` boolean parameter.
- [ ] 5.3 Behavior test: `scene_create` refuses when an editable stage is present,
  and `force=true` replaces it.
- [ ] 5.4 Regression test: `load_scene_from_stage(..., allow_empty=True)` on a
  `/World`-only stage returns an empty `Scene`; and a created scene does not fall
  back to the analytic head (`_rebake_if_needed`).

## 6. Docs

- [ ] 6.1 Add `scene_create` to the README MCP tool list.

## 7. Validate & land

- [ ] 7.1 `openspec validate mcp-scene-create --strict`.
- [ ] 7.2 Run hostless tests + `ruff check src/` on changed files.
- [ ] 7.3 Codex (or review-subagent fallback) pre-merge review; fold findings.
