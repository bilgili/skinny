...
- [~] 5.4 Per-material shade *dispatch* over each material queue. Mechanism GPU-verified — counting-sort (`build_args`+`scatter`) → per-material `vkCmdDispatchIndirect` (`IndirectPaintPass` == direct fallback, test 9.3) + the per-material shade *pipelines* + compile cache (6.4). PATH-LOOP INTEGRATION IN PROGRESS (incremental): **step A DONE** — `wfPathBounce` split into `wfPathIntersect` (material-free trace, ~250 KB) + `wfPathShade` (materials, ~2.8 MB) via a per-lane hit buffer; A/B unchanged (`test_wavefront_path_ab`). REMAINING: **step A2** — split `wfPathShade` per material type so a flat-only scene compiles a small flat shade kernel (no skin/python imports → fixes the MoltenVK flaky Metal compile of the 2.8 MB kernel); needs extracting `allLightsNEE`/`neeLightEstimator` from `integrators/path.slang` into a material-free `nee.slang` (megakernel A/B must hold), per-type bounce bodies, and scene-type-aware kernel building. **step B** — route hits through the per-material queues + indirect dispatch (coherence). The path tracer is correct + A/B-verified throughout.
