...
- [x] 1.1 Capture the current refusal matrix per front-end as test fixtures:
...
- [x] 2.1 Add `src/skinny/bringup.py`: `plan_bringup(args, prog, persisted=None)`
...
- [x] 2.2 Add `BringupPlan.create(window=None, width, height, gpu_preference=None,
      ...
      at the call sites.
- [x] 2.3 Add hostless `tests/test_bringup.py`: guard matrix (integrator ×
...
- [x] 2.4 `ruff check src/` clean; hostless suite green with no front-end
      migrated yet.

## 3. Migrate skinny-render (no persistence, no deferral)

- [x] 3.1 Replace `headless.py` `main()` bring-up region with
...
- [x] 5.3 Refusal-parity check incl. the persisted-sppm + forced-megakernel
      case; interactive startup behavior unchanged.

## 6. Migrate skinny-gui (deferred to Qt render thread)

- [x] 6.1 Replace `ui/qt/app.py` `main()` bring-up region with
...
- [x] 6.2 Move `ui/qt/viewport.py` `_build_renderer` (~70–104) onto
...
- [x] 6.3 Refusal-parity check vs fixtures; offscreen Qt smoke
      (`QT_QPA_PLATFORM=offscreen`) green.

## 7. Docs + close-out

- [x] 7.1 Update `docs/Architecture.md` with the bring-up module (sequence,
      staging, per-front-end knobs); sweep `README.md`/`CLAUDE.md` for any
      per-front-end resolution wording that moved.
- [x] 7.2 Full hostless suite + `ruff check src/` green;
