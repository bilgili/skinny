...
- [ ] 1.1 `wavefront/wf_records.slang`: change the stub gate from
...
- [ ] 1.2 `integrators/path_record_common.slang` + `bindings.slang`: extend the
...
- [ ] 1.3 `metal_wavefront.py`: `records_active` ctor flag on the path pass →
...
- [ ] 1.4 Post-build reflected slot check: after session link, count each entry
...
- [ ] 1.5 Guarded GPU smoke: neural+records pass builds on this host; log the
...
- [ ] 2.1 `renderer.py`: replace the `descriptor_sets is not None` gate in
...
- [ ] 2.2 `_ensure_wf_record_drain`: Metal branch — allocate the drain target +
...
- [ ] 2.3 `_drain_wavefront_records`: backend split — Vulkan path unchanged;
      ...
      exactly like Vulkan.
- [ ] 2.4 `recordMode` in the MSL uniform pack: confirm `_pack_uniforms_msl`
...
- [ ] 2.5 Unit tests (no GPU): record-source resolution arms `_wf_record_active`
...
- [ ] 3.1 Guarded GPU test: records-enabled Metal wavefront render on the flat
      Cornell scene drains records — count > 0, finite contributions, stack
      bounded (≤ REC_MAX_BOUNCES per lane), bytes parse via
      `records_from_buffer`.
- [ ] 3.2 Guarded A/B: same scene/config/sample budget recorded on Metal and
...
- [ ] 3.3 Guarded bit-identity: Metal wavefront render with records off is
      byte-identical to the pre-change render (records build never constructed).
- [ ] 3.4 Guarded end-to-end: fully-on-Metal online loop — wavefront path +
...
- [ ] 3.5 Measure path-pass frame time records-on vs records-off on the test
      scene; note the overhead number in the change.

## 4. Docs + housekeeping

- [ ] 4.1 `docs/NeuralGuiding.md`: drop the "record drain remains Vulkan-only"
      caveat; document the Metal records build flavor + fully-on-Metal recipe.
- [ ] 4.2 `docs/Wavefront.md` Metal section + `docs/Architecture.md` slot-cap
      and drain notes (36/37 on Metal, SKINNY_METAL_RECORDS).
- [ ] 4.3 `README.md` online-training platform notes + `CHANGELOG.md` entry.
- [ ] 4.4 `ruff check src/`, `node docs/diagrams/embed_code.cjs --check`,
