...
- [ ] 1.1 Probe the pinned slangpy build: can an upload-heap (`MemoryType.upload`)
...
- [ ] 1.2 `metal_compute.StorageBuffer`: add `shared: bool = False` ctor kwarg —
...
- [ ] 1.3 `MetalContext`: add `supports_shared_memory` capability flag (probe
      unified memory; conservative `False` when unknown); leave
      `supports_external_memory`/`supports_external_semaphore` `False`.
- [ ] 1.4 GPU-marked test: shared-storage buffer in-place host write observed by
...
- [ ] 2.1 New `sampling/neural_handoff_interop_metal.py`:
...
- [ ] 2.2 Availability guard `metal_interop_available()` — requires a Metal
      shared-capable context; failure message names `--neural-handoff file`.
- [ ] 2.3 `make_publisher("interop", ...)` dispatches by `weights_buffer` backend:
...
- [ ] 2.4 Unit tests (no GPU): staging/swap version protocol, precision
...
- [ ] 3.1 Allocate bindings 33/34 with `shared=True` on Metal when
...
- [ ] 3.2 `enable_online_training`: extend the `kind == "interop"` block — on
      ...
      Metal publisher.
- [ ] 3.3 Confirm the frame-end swap call-site ordering on the Metal wavefront
...
- [ ] 3.4 `cli_common.py`: `--neural-handoff` help text mentions Metal UMA
      interop; validation unchanged (same two kinds).

## 4. Parity verification

- [ ] 4.1 GPU-marked end-to-end test (`tests/test_neural_interop_metal.py`):
...
- [ ] 4.2 Measure the swap-frame cost of the staged copy (publish-frame vs
      steady-state frame time); note the number in the change; split
      weights/biases across two boundaries only if visible.
- [ ] 4.3 Guard test: `--neural-handoff interop` on a host with neither CUDA nor
      Metal UMA raises `NotImplementedError` naming the file fallback (mock the
      availability probes).

## 5. Docs + housekeeping

- [ ] 5.1 `docs/NeuralGuiding.md`: handoff section gains the Metal UMA interop
      path (publish→stage, swap→in-place write, sync rationale).
- [ ] 5.2 `docs/Architecture.md`: capability-flag table adds
      `supports_shared_memory`; note bindings 33/34 shared-storage mode on
      Metal.
- [ ] 5.3 `README.md` flag docs + `CHANGELOG.md` entry.
- [ ] 5.4 `ruff check src/`, full `pytest -m 'not gpu'` sweep, then the guarded
