...
The human-readable compatibility-matrix tables (CLAUDE.md and README) SHALL be retained as documentation **of** the predicate, with the predicate module as the stated source of truth — superseding the prior convention that code mirrors the documented matrix. A hostless doc-sync check SHALL assert that key envelope facts derived from the predicate (at minimum: the wavefront-only integrator set, and the axes refused under spectral) are stated in the documented tables, so envelope edits that skip the docs fail a test.

#### Scenario: doc drift fails the check
- **WHEN** an envelope rule covered by the doc-sync check changes in the predicate but the documented compatibility tables are not updated
