...
ReSTIR DI's emissive-triangle light candidates SHALL be drawn through the same
sampling seam (`light.samplePoint` / `selectionPdf`), so they use the
power-weighted distribution with no ReSTIR-specific change. The distribution
SHALL also apply at secondary bounces, which ReSTIR (primary-hit only) does not
cover.
...
- **THEN** those candidates are selected with the power-weighted probability `p_i`, not uniform-by-index, with no change to the ReSTIR reservoir/RIS code
