...
Persist and recall the last-used directory per file-loader category (`model`,
`ibl`, `lens`) so file-open dialogs reopen where the user last worked, with
type-specific default fallbacks, shared across front-ends.
