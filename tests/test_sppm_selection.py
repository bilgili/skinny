...
def test_state_hash_includes_integrator_index():
    # The state hash derives from the params.py registry (change
    # param-registry-accumulation-reset): integrator_index must be a
    # resets_accumulation param so cycling to/from SPPM resets accumulation.
    ...
    assert spec.resets_accumulation, \
        "integrator_index must reset accumulation (registry flag) on switch"
